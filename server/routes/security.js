  ...
  try {
    ...
    mongo(async db => {
      ...
      if (userExists) {
        ...
        const result = await db.collections('users').insertOne(newUser)
...
router.post('/verify/users/:email/security-questions', (req, res, next) => {
  try {
    ...
    mongo (async db => {
      ...
      console.log('Selected User', user)
      if (securityQuestions[0].answer !== user.selectedSecurityQuestions[0].answer || securityQuestions[1].answer !== user.selectedSecurityQuestions[1] || securityQuestions[3].answer !== user.selectedSecurityQuestions[3].answer) {
        const err = new Error('Unauthorized')
        err.status = 401
